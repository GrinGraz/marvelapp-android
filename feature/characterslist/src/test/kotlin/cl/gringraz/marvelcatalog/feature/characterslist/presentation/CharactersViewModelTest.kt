...
class CharactersViewModelTest {
    ...
    inner class GetMarvelCharactersByViewModel {
        ...
        inner class RequestAndMappingIsLoading {
            ...
            @Test
            @DisplayName("Then the view model gets a marvel characters Initial ui model")
            fun execute() = runTest(testCoroutineDispatcher) {
                sut.getMarvelCharacters()
                assertEquals(MarvelCharactersListUiState.Initial, sut.marvelCharactersUiState.value)
            }
    ...
    @Test
    @DisplayName("Given a initial listing type string when set listing type then returns its enum representation")
    fun `Test set listing type`() {
        sut.setListingType("explore")
        assertEquals(ListingType.Explore, sut.listType)
        sut.setListingType("search")
        assertEquals(ListingType.Search, sut.listType)
        sut.setListingType("else")
        assertEquals(ListingType.Explore, sut.listType)
    }

    @Test
    @DisplayName("Given a initial value when compare and set previous query then returns if set different query")
    fun `Test compare and set previous query`() {
        assertEquals(true, sut.compareAndSetPreviousQuery("query"))
        assertEquals(false, sut.compareAndSetPreviousQuery("query"))
    }
}
